...
function DataManager({ userData, onDataUpdate }) {
  ...
  const handleSheetConnect = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "Failed to fetch sheet. Make sure the sheet is shared publicly (Anyone with the link can view).",
        );
      }
  ...
  const handleClearAll = () => {
    if (
      confirm(
        "Are you sure you want to clear all datasets and chat history? This cannot be undone.",
      )
    ) {
      // Clear datasets
      setDatasets([]);
      ...
      {isAddingManual && (
        <div className="manual-data-form">
          ...
          <textarea
            ...
            onChange={(e) =>
              setManualData((prev) => ({ ...prev, csvText: e.target.value }))
            }
      ...
      {isAddingSheet && (
        <div className="manual-data-form sheet-connect-form">
          ...
          <input
            ...
            onChange={(e) =>
              setSheetData((prev) => ({ ...prev, name: e.target.value }))
            }
            className="manual-data-name"
          />
          <input
            ...
            value={sheetData.url}
            onChange={(e) =>
              setSheetData((prev) => ({ ...prev, url: e.target.value }))
            }
            className="manual-data-name"
          />
          <button
              ...
              <div className="dataset-header">
                <div className="dataset-info">
                  ...
                  <p className="dataset-meta">
                    ...
                    {Object.keys(dataset.data[0] || {}).length} columns
                    {dataset.uploadedAt &&
                ...
                <div className="dataset-actions">
                  {dataset.source === "google-sheets" && (
                    ...
                    >
                      <RefreshCw
                        size={18}
                        className={loadingSheet ? "spinning" : ""}
                      />
                    </button>
                  )}
                  <button
                ...
                <div className="dataset-preview">
                  ...
                  {dataset.data.length > 5 && (
                    <p className="preview-note">
                      Showing first 5 of {dataset.data.length} rows
                    </p>
                  )}
                </div>
              )}
