  ...
  if (active && payload && payload.length) {
    return (
      <div style={{
        ...
        <p style={{ color: '#f0f0f5', fontWeight: 500, marginBottom: '4px' }}>{label}</p>
        ...
        <LineChart data={chart.data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
          ...
          {chart.data[0]?.value2 !== undefined && (
            <Line 
              ...
              strokeWidth={2}
              dot={{ fill: chart.colors?.[1] || '#00e5ff', strokeWidth: 0, r: 4 }}
              activeDot={{ r: 6 }}
              name="Series 2"
            />
          )}
        ...
        <AreaChart data={chart.data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
          <defs>
            <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={chart.colors?.[0] || '#3d5afe'} stopOpacity={0.3}/>
              <stop offset="95%" stopColor={chart.colors?.[0] || '#3d5afe'} stopOpacity={0}/>
            </linearGradient>
            <linearGradient id="colorValue2" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={chart.colors?.[1] || '#00e5ff'} stopOpacity={0.3}/>
              <stop offset="95%" stopColor={chart.colors?.[1] || '#00e5ff'} stopOpacity={0}/>
            </linearGradient>
          </defs>
          <CartesianGrid strokeDasharray="3 3" stroke="#2a2a38" />
          <XAxis dataKey="name" {...commonAxisProps} />
